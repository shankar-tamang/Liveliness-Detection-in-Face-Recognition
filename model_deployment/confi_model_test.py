...
try:
    ...
    # model = load_model('liveliness_detection_model.h5')  # Use this if you're using the other model
...
def predict_liveness(image):
    ...
    return prediction[0][0]  # Return the confidence for the "Live" class

# Main function for image and video processing
def main():
    input_type = int(input("Enter 1 for image or 2 for video: "))
    threshold = 0.45 # Set your desired threshold here
    ...
    if input_type == 1:  # Image
        ...
        for face in faces:
            ...
            # Predict liveness and get confidence
            confidence = predict_liveness(face_roi)
            label = "Live" if confidence > threshold else "Spoof"

            # Display confidence
            confidence_text = f"Confidence: {confidence:.2f}"
            cv2.putText(image, confidence_text, (x1, y1 - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
        ...
        while True:
            ...
            for face in faces:
                ...
                # Display confidence
                confidence_text = f"Confidence: {confidence:.2f}"
                cv2.putText(frame, confidence_text, (x1, y1 - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
